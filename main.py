...
AGENT_CODES = {
    ...
    "23": "0e38b510-41a8-5780-5e8f-568b2a4f2d6c", # Iso
    "24": "efba5359-4016-a1e5-7626-b1ae76895940", # Vyse
    "25": "df1cb487-4902-002e-5c17-d28e83e78588", # Waylay
}
...
def get_Settings():  # stores the Agent Code and the player Region
    ...
    AGENT_CODES["0"] = AGENT_CODES[
        str(random.randint(1, len(AGENT_CODES) - 1))
    ]  # Intitialize the random thingy
    if not os.path.exists(os.getcwd() + "\\Settings.txt"):
        if input("Would you like to change settings? y/n\n").lower() == "y":
            REGIONCODE = input(
                "Enter Region:\n[1] North America\n[2] Latin America\n[3] Brazil\n[4] Europe\n[5] Asia Pacific\n[6] Korea\n[7] Pbe\nEnter Number: "
            )
            AGENTCODE = AGENT_CODES[
                input(
                    "Enter Agent:\n[0] Random \n[1] Jett\n[2] Reyna\n[3] Raze\n[4] Yoru\n[5] Phoenix\n[6] Neon\n[7] Breach\n[8] Skye\n[9] Sova\n[10] Kayo\n[11] Killjoy\n[12] Cypher\n[13] Sage\n[14] Chamber\n[15] Omen\n[16] Brimstone\n[17] Astra\n[18] Viper\n[19] Fade\n[20] Harbor\n[21] Gekko\n[22] Deadlock\n[23] Iso\n[24] Vyse\n[25] Waylay\nEnter Number: "
                )
            ]
            LOCK_DELAY = input(
                "Choose a lock in delay in seconds (0.1 for 100 miliseconds, default locks during main menu countdown):\n"
            )
            with open(
                os.getcwd() + "\\Settings.txt", "w"
            ) as f:  # Updates the file with the settings
                f.write(REGIONCODE + "\n" + AGENTCODE + "\n" + LOCK_DELAY)
    else:
        try:
            ...
            print(
                "Settings file doesn't exist, Please try again. (If it's first time you must edit settings)"
            )
            get_Settings()
...
def try_Lock():
    ...
    if not "VALORANT.exe" in (
        p.name() for p in psutil.process_iter()
    ):  # Checks windows process to see if valorant is running
    ...
    try:
        client = Client(region=REGIONS[REGIONCODE])
        # type: ignore
    except ValueError:
    ...
    while RUNNING:
        ...
        try:
            ...
            # Checks to see if your online in Valorant
            matchID = client.pregame_fetch_match()["ID"]
            # Grabs the pre-game ID
            print(matchID)

            print(sessionState)

            if (
                sessionState == "PREGAME" and matchID not in SEEN_MATCHES
            ):  # Making sure not a game thats been seen before
                ...
                client.pregame_select_character(AGENTCODE)
                # type: ignore
                ...
                client.pregame_lock_character(AGENTCODE)
                # type: ignore
...
"""
    ...
    "7ca9995d-4f35-dae1-acfb-25b8a2fb1fa0": "Shooting Range",
    "2bee0dc9-4ffe-519b-1cbd-7fbe763a6047": "Haven"

    If i want to add map detection 
    favorite agents by map 
